...
function GoalPredictionPage() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const response = await api.post(
				'/api/predict-halftime-goals',
				requestBody
			)
			setResults(response.data)
			toast.success('Goal prediction completed successfully!')
		} catch (error) {
			console.error('Error predicting halftime goals:', error)
			setError(
				error.response?.data?.error || 'Failed to predict halftime goals'
			)
			toast.error('Failed to predict halftime goals')
		} finally {
			setLoading(false)
		}
	}
	...
	// Prepare donut chart data if results are available
	...
	const chartOptions = {
		...
		plugins: {
			...
			title: {
				display: true,
				text: 'Halftime Goal Probabilities',
			},
			tooltip: {
				callbacks: {
	...
	const donutOptions = {
		...
		plugins: {
			...
			title: {
				display: true,
				text: 'Halftime Goal Distribution',
			},
			tooltip: {
				callbacks: {
					label: function (context) {
					...
					<Grid container spacing={3}>
						...
						<Grid item xs={12}>
							<Button
								...
								sx={{ mt: 2 }}
							>
								{loading ? <CircularProgress size={24} /> : 'Predict Goals'}
							</Button>
						</Grid>
					</Grid>
				</form>
			...
			{results && !loading && (
				<Box sx={{ mt: 4 }}>
					...
					<Grid container spacing={4}>
						...
						<Grid item xs={12} md={6}>
							<Card>
								<CardHeader title='Goal Distribution Visualization' />
								<CardContent>
									<Box height={300}>
											...
											([goals, probability]) => (
												<Card
													...
													sx={{
														...
														bgcolor:
															goals ===
															results.most_likely_outcome.split(' ')[0]
																? 'rgba(76, 175, 80, 0.1)'
																: 'inherit',
													}}
												>
...
export default GoalPredictionPage
